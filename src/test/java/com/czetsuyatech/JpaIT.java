...
public class JpaIT {
	...
	public static Archive<?> getEarArchive() {
		...
		File f = new File("./target/jpa-to-cdi-events.war");
		if (f.exists() == false) {
			throw new RuntimeException("File " + f.getAbsolutePath() + " does not exist.");
		}
		WebArchive war = ShrinkWrap.create(ZipImporter.class, "jpa-to-cdi-events.war").importFrom(f)
				.as(WebArchive.class);

		// Add the package containing the test classes:
		war.addPackage("com.czetsuyatech");
